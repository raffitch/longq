...
export type HormoneSeverity = GeneralSeverity;
...
const HormonesCard: React.FC<HormonesCardProps> = ({ data }) => {
  const grouped = GENERAL_SEVERITY_ORDER.reduce<Record<GeneralSeverity, HormoneItem[]>>(
    (acc, severity) => {
      acc[severity] = [];
      return acc;
    },
    {} as Record<GeneralSeverity, HormoneItem[]>,
  );
