...
def parse_file(kind: str, pdf_path: Path) -> Tuple[str, Any]:
    ...
    if kind == "nutrition":
        if parse_nutrition_pdf is None:
            raise ValueError("Nutrition parser not available; ensure parse_nutrition_pdf is importable.")
        data = parse_nutrition_pdf(str(pdf_path))
        if not isinstance(data, dict) or "items" not in data:
            raise RuntimeError("Nutrition parser returned unexpected shape; expected dict with 'items'.")
        return ("nutrition-v1", data)

    if kind == "hormones":
        if parse_hormones_pdf is None:
            raise ValueError("Hormones parser not available; ensure parse_hormones_pdf is importable.")
        data = parse_hormones_pdf(str(pdf_path))
        if not isinstance(data, dict) or "items" not in data:
            raise RuntimeError("Hormones parser returned unexpected shape; expected dict with 'items'.")
        return ("hormones-v1", data)

    if kind == "heavy-metals":
        if parse_heavy_metals_pdf is None:
            raise ValueError("Heavy-metals parser not available; ensure parse_heavy_metals_pdf is importable.")
        data = parse_heavy_metals_pdf(str(pdf_path))
        if not isinstance(data, dict) or "items" not in data:
            raise RuntimeError("Heavy-metals parser returned unexpected shape; expected dict with 'items'.")
