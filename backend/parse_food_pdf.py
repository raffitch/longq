...
CATEGORY_NAMES = [
    "Dairy","Eggs","Fruits","Grains","Legumes","Meats","NutsSeeds","Seafood","Vegetables","Wheat",
    "HeavyMetals","Lectins"
]
CANON = {
    "dairy": "Dairy",
    ...
    for b in d.get("blocks", []):
        for ln in b.get("lines", []):
            for sp in ln.get("spans", []):
                ...
                if not t: continue
                x0, y0, x1, y1 = sp["bbox"]
                out.append({"t": t, "x0": x0, "y0": y0, "x1": x1, "y1": y1, "size": sp.get("size", 0.0)})
...
def cluster_lines(spans, y_tol=Y_LINE_TOL):
    lines = []
    for sp in spans:
        if not lines: lines.append([sp]); continue
        last = lines[-1]
        ...
        if abs(sp["y0"] - y_ref) <= y_tol:
            ...
            lines.append([sp])
    if lines: lines[-1].sort(key=lambda s: s["x0"])
    return lines
...
def group_headers_into_rows(headers):
    rows = []
    for h in headers:
        if not rows: rows.append([h]); continue
        last_row = rows[-1]
        ...
        if abs(h["y0"] - y_ref) <= ROW_Y_TOL:
            ...
            rows.append([h])
    if rows: rows[-1].sort(key=lambda r: r["x0"])
    return rows
...
def build_row_boxes(rows, page_w: float):
    ...
    for row in rows:
        ...
        if len(row) == 1:
            ...
            edges = [0.0] + [(row_sorted[i]["x0"] + row_sorted[i+1]["x0"]) / 2.0 for i in range(len(row_sorted)-1)] + [float(page_w)]
...
def find_scores_in_box(lines, x0, x1, y0, y1):
    ...
    for ln in lines:
        ...
        if not (y0 <= ly < y1): continue
        for s in ln:
            if s["x1"] < x0 or s["x0"] > x1: continue
...
def cluster_score_rows(scores, tol=ROW_CLUSTER_TOL):
    rows = []
    for sc in scores:
        if not rows: rows.append([sc]); continue
        last = rows[-1]
        ...
        if abs(sc["y"] - y_ref) <= tol:
            ...
            rows.append([sc])
    if rows: rows[-1].sort(key=lambda s: s["x"])
    return rows

def build_row_bands(row_centers, cat_y0, cat_y1):
    bands = []
    if not row_centers: return bands
...
def smart_join(parts):
    ...
    out = parts[0]
    for nxt in parts[1:]:
        if out.endswith("-"): out = out + nxt.lstrip()
        elif nxt.startswith((")", ",", ".", ":", ";")): out = out + nxt
        elif nxt.startswith(("'", '"')): out = out + nxt
        elif nxt.startswith((")",)) or out.endswith(("(", "/")): out = out + nxt
        else: out = out + " " + nxt
    out = re.sub(r"\s+\)", ")", out)
    out = re.sub(r"\(\s+", "(", out)
    return out.strip(" •·-—")
...
def collect_label_window(lines, y_top, y_bot, x_left, x_right, expected_set):
    ...
    for ln in lines:
        ly = min(s["y0"] for s in ln)
        if not (y_top <= ly < y_bot): continue
        for s in ln:
            if s["x1"] < x_left or s["x0"] > x_right: continue
            t = s["t"]
            if SCORE_RE.match(t): continue
            if canon_cat(t) in expected_set: continue
            parts.append(t)
    return smart_join(parts)

def quantiles(values, qs):
    if not values: return [0.0]*len(qs)
    vs = sorted(values)
    out = []
    for q in qs:
...
def kmeans1d_median(xs, k=4, iters=20):
    if not xs: return []
    xs_sorted = sorted(xs)
    if k <= 1: return [statistics.median(xs_sorted)]
...
def header_has_items(lines, header, page_w: float, header_row, all_headers):
    ...
    if len(row) == 1:
        ...
        edges = [0.0] + [(row[i]["x0"] + row[i+1]["x0"]) / 2.0 for i in range(len(row)-1)] + [float(page_w)]
        hi = next(i for i, h in enumerate(row) if h is header)
        x0, x1 = edges[hi], edges[hi+1]
    y0 = header["y0"]; y1 = y0 + HEADER_VALIDATE_Y_RANGE
    for ln in lines:
        ly = min(s["y0"] for s in ln)
        if not (y0 <= ly < y1): continue
        for s in ln:
...
def decide_order_mode(items_xy, row_groups):
    if not items_xy: return "row"
    xs = [x for (x, y) in items_xy]
    centers = kmeans1d_median(xs, k=4, iters=25)
    def col_idx(x): return min(range(len(centers)), key=lambda i: abs(x - centers[i]))
